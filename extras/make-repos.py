		...
		for line in conf.readlines():
			if not "#" in line:
				if "SATELLITE_FQDN" in line:
					line = line.strip()
					value = line.split('=')
					if len(value[1]) > 0: SATELLITE_FQDN = value[1]
				if "SATELLITE_USERNAME" in line:
					line = line.strip()
					value = line.split('=')
					if len(value[1]) > 0: SATELLITE_USERNAME = value[1]
				if "SATELLITE_PASSWORD" in line:
					line = line.strip()
					value = line.split('=')
					if len(value[1]) > 0: SATELLITE_PASSWORD = value[1]
				if "OUTPUT_DIR" in line:
					line = line.strip()
					value = line.split('=')
					if len(value[1]) > 0: OUTPUT_DIR = value[1]
...
if __name__ == "__main__":
	...
	if conf:
		...
		SATELLITE_FQDN, SATELLITE_USERNAME, SATELLITE_PASSWORD, OUTPUT_DIR = parse_config(conf)
	...
	if check_createrepo():
		...
		for dir in os.listdir(OUTPUT_DIR):
			...
			subprocess.call(['/usr/bin/createrepo','--workers=4', this_dir], stdout=devnull, stderr=devnull)
