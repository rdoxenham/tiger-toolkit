...
def show_current_ip():
    try:
        ipaddress_ps = subprocess.Popen(
            ['/usr/bin/facter', 'ipaddress_eth0'], stdout=subprocess.PIPE)
        current_ip, err = ipaddress_ps.communicate()
        netmask_ps = subprocess.Popen(
            ['/usr/bin/facter', 'netmask_eth0'], stdout=subprocess.PIPE)
...
def run_ip_update(new_ip, new_netmask, new_gateway, current_ip):
    ...
    try:
        subprocess.call(
            ['/bin/sed', '-i', 's/IPADDR=.*/IPADDR=' + new_ip + '/g',
             '/etc/sysconfig/network-scripts/ifcfg-eth0'], stdout=devnull, stderr=devnull)
        subprocess.call(
            ['/bin/sed', '-i', 's/NETMASK=.*/NETMASK=' + new_netmask + '/g',
             '/etc/sysconfig/network-scripts/ifcfg-eth0'], stdout=devnull, stderr=devnull)
        subprocess.call(
            ['/bin/sed', '-i', 's/GATEWAY=.*/GATEWAY=' + new_gateway + '/g',
             '/etc/sysconfig/network-scripts/ifcfg-eth0'], stdout=devnull, stderr=devnull)
    except:
    ...
    try:
        db = MySQLdb.connect(
            host="localhost", user="root", passwd="redhat", db="keystone")
        cur = db.cursor()
    ...
    try:
        for file in file_list:
            subprocess.call(
                ['/bin/sed', '-i', 's/' + current_ip.strip() + '/' + new_ip.strip() + '/g', file])
    except:
